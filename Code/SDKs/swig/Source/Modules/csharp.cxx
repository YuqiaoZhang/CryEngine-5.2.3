...
class CSHARP :public Language {
  ...
  bool no_exceptions; // Flag usage of "throw" statements
  bool no_include_guards; // use 'pragma once' instead of oldschool include guards
  ...
  String *pch_file;		// C++ pre-compiled header
...
public:
  ...
  virtual void main(int argc, char *argv[]) {
	...
	if (strcmp(argv[i], "-dllimport") == 0) {
	...
	} else if (strcmp(argv[i], "-pch-file") == 0) {
		if (argv[i + 1]) {
			pch_file = NewString("");
			Printf(pch_file, argv[i + 1]);
			Swig_mark_arg(i);
			Swig_mark_arg(i + 1);
			i++;
		}
	  ...
	  Swig_mark_arg(i);
	  no_exceptions = true;
	} else if (strcmp(argv[i], "-fno-include-guards") == 0) {
		Swig_mark_arg(i);
  ...
  virtual int top(Node *n) {
    ...
    if (directorsEnabled()) {
	  ...
	  if(no_include_guards) {
		  Printf(f_directors_h, "#pragma once\n");
	  } else {
	...
	Printv(f_im, "implements ", imclass_interfaces, " ", NIL);
	  ...
	  if(output_file)
	    Replaceall(imclass_class_code, "$outPrefix", SwigType_manglestr(output_file));
	...
	Printv(f_module, "implements ", module_interfaces, " ", NIL);
	  ...
	  if(output_file) {
	    Replaceall(module_class_code, "$outPrefix", SwigType_manglestr(output_file));
		Replaceall(module_class_constants_code, "$outPrefix", SwigType_manglestr(output_file));
	  }
  ...
  virtual int insertDirective(Node *n) {
	...
	if(output_file)
	    Replaceall(code, "$outPrefix", SwigType_manglestr(output_file));
  ...
  virtual int classHandler(Node *n) {
    ...
    if (proxy_flag) {
	  ...
	  if(output_file) {
	    Replaceall(proxy_class_def, "$outPrefix", SwigType_manglestr(output_file));
		Replaceall(proxy_class_code, "$outPrefix", SwigType_manglestr(output_file));
		Replaceall(proxy_class_constants_code, "$outPrefix", SwigType_manglestr(output_file));
	  }
      if (!has_outerclass)
  ...
  void substituteClassnameSpecialVariable(SwigType *classnametype, String *tm, const char *classnamespecialvariable) {
	...
	if (anonymous_enum) {
	  ...
	  replacementname = NewStringf("%s_SWIGTYPE%s", SwigType_manglestr(output_file), SwigType_manglestr(classnametype));
  ...
  void emitTypeWrapperClass(String *classname, SwigType *type) {
	...
	if(output_file)
	    Replaceall(swigtype, "$outPrefix", SwigType_manglestr(output_file));
