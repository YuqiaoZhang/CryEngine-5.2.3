...
public:
  ...
  bool assignTo(const yasli::Serializer& ser) const override 
	...
	void increment(PropertyTree* tree, int mouseDiff, Modifier modifier) override
	{
		...
		if (hasMinMax())
		{
			add = (maxValue() - minValue()) * double(mouseDiff) / double(widgetRect(tree).width());
		}
		else
		{
		...
		value_ = clamp(value_ + add, hardMin_, hardMax_);
	}

	void addValue(PropertyTree* tree, double value) override
	{
		tree->model()->rowAboutToBeChanged(this);
		value_ = clamp(value_ + value, hardMin_, hardMax_);
		tree->model()->rowChanged(this, true);
	}

	bool hasMinMax() const
	{
		return hardMin_ != std::numeric_limits<Type>::lowest() && hardMax_ != std::numeric_limits<Type>::max();
	}

	double minValue() const override { return hasMinMax() ? hardMin_ : -DBL_MAX; }
	double maxValue() const override { return hasMinMax() ? hardMax_ : DBL_MAX; }
