...
{
	gEnv->pConsole->UnregisterVariable("game_pl_movementSpeed", true);
	gEnv->pConsole->UnregisterVariable("game_pl_boostMultiplier", true);
...
void CMovementExtension::PostInit(IGameObject* pGameObject)
{
	...
	REGISTER_CVAR2("game_pl_movementSpeed", &m_movementSpeed, 20.0f, VF_NULL, "Player movement speed.");
	REGISTER_CVAR2("game_pl_boostMultiplier", &m_boostMultiplier, 2.0f, VF_NULL, "Player boost multiplier.");
}
