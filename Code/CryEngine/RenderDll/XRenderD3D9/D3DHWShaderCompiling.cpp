...
#if !defined(_RELEASE)
		nFlags |= D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION;
		char Win32FileName[MAX_PATH + 1];
		::SHGetFolderPathA(NULL, CSIDL_PERSONAL | CSIDL_FLAG_CREATE, NULL, 0, Win32FileName);
		::strcat(Win32FileName, "\\CryEngine@");
		::strcat(Win32FileName, GetName());
		::strcat(Win32FileName, ".hlsl");
		HANDLE hWin32File = ::CreateFileA(Win32FileName, FILE_WRITE_DATA, 0U, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		assert(hWin32File != INVALID_HANDLE_VALUE);
		DWORD NumberOfBytesWritten;
		::WriteFile(hWin32File, prog_text, ::strlen(prog_text), &NumberOfBytesWritten, NULL);
		assert(NumberOfBytesWritten == ::strlen(prog_text));
		::CloseHandle(hWin32File);
		hr = ::D3DCompile(prog_text, ::strlen(prog_text), Win32FileName, NULL, NULL, pFunCCryName, szProfile, nFlags, 0, (ID3DBlob**)ppShader, (ID3DBlob**)ppErrorMsgs);
#else
		hr = ::D3DCompile(prog_text, ::strlen(prog_text), NULL, NULL, NULL, pFunCCryName, szProfile, nFlags, 0, (ID3DBlob**)ppShader, (ID3DBlob**)ppErrorMsgs);
#endif
...
bool CAsyncShaderTask::CompileAsyncShader(SShaderAsyncInfo* pAsync)
{
	...
	else
	{
		...
		::SHGetFolderPathA(NULL, CSIDL_PERSONAL | CSIDL_FLAG_CREATE, NULL, 0, Win32FileName);
		::strcat(Win32FileName, "\\CryEngine@");
		::strcat(Win32FileName, Name);
		::strcat(Win32FileName, ".hlsl");
		HANDLE hWin32File = ::CreateFileA(Win32FileName, FILE_WRITE_DATA, 0U, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		auto huhu = ::GetLastError();
		assert(hWin32File != INVALID_HANDLE_VALUE);
		DWORD NumberOfBytesWritten;
		::WriteFile(hWin32File, pAsync->m_Text.c_str(), pAsync->m_Text.size(), &NumberOfBytesWritten, NULL);
		assert(NumberOfBytesWritten == pAsync->m_Text.size());
		::CloseHandle(hWin32File);
		hr = ::D3DCompile(pAsync->m_Text.c_str(), pAsync->m_Text.size(), Win32FileName, NULL, NULL, pAsync->m_Name.c_str(), pAsync->m_Profile.c_str(), nFlags, 0, (ID3DBlob**)&pAsync->m_pDevShader, (ID3DBlob**)&pAsync->m_pErrors);
#else
		hr = ::D3DCompile(pAsync->m_Text.c_str(), pAsync->m_Text.size(), NULL, NULL, NULL, pAsync->m_Name.c_str(), pAsync->m_Profile.c_str(), nFlags, 0, (ID3DBlob**)&pAsync->m_pDevShader, (ID3DBlob**)&pAsync->m_pErrors);
#endif
